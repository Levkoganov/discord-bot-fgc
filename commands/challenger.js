  ...
  async execute(interaction) {
    ...
    if (hasRole && isCurrentGameChampion) {
      ...
      const filter = async (i) => {
        // Check users ID
        if (i.user.id === champion.id || i.user.id === challenger.id)
          return true;
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === champion.username) {
          ...
          if (champion_score === rounds) {
            ...
            if (!championChannel) {
              await i.editReply({content: "please set a channel with '/set-channel-champion'."});
        ...
        if (i.customId === challenger.username) {
          ...
          if (challenger_score === rounds) {
            ...
            await manageRole(interaction, champion.id, challengerMember)
