    ...
    try {
      ...
      if (interaction.member.permissions.has(PermissionsBitField.Flags.Administrator) || hasModRole) {
        ...
        if (previousChampData.length > 0) {
          ...
          // Role assignment
          hasChampionRole = await manageRole(interaction, previousChampData[0].userId, selectedUser, championRoleName)
          roleOutPutResult = `${hasChampionRole ? "" : 'champion DID NOT get role! make sure "KOTH - Champion" rolename exist.'}`
